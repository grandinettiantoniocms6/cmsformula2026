	  ...
	  <h3>
        ...
        @else
          @if ($crud->hasAccess('list'))
              <small><a href="{{ url($crud->route) }}?user_id={{ $crud->entry->user_id }}" class="d-print-none font-sm"><i class="la la-angle-{{ config('backpack.base.html_direction') == 'rtl' ? 'right' : 'left' }}"></i> {{ trans('backpack::crud.back_to_all') }} <span>{{ $crud->entity_name_plural }}</span></a></small>
          @endif
        @endif
	  </h3>
      <hr>
