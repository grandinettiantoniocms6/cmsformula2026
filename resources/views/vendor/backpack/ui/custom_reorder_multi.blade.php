...
<div class="container-fluid">
    <h2>
        ...
        @if ($crud->hasAccess('list'))
          <small><a href="{{ url($crud->route) }}?block_id={{ request()->get('block_id') }}&block={{ request()->get('block') }}&page_id={{ request()->get('page_id') }}" class="d-print-none font-sm"><i class="la la-angle-double-left"></i> {{ trans('backpack::crud.back_to_all') }} <span>{{ $crud->entity_name_plural }}</span></a></small>
        @endif
    </h2>
</div>
@endsection
...
function tree_element($entry, $key, $all_entries, $crud)
{
    if (! isset($entry->tree_element_shown)) {
        ...
        switch ($crud->get('reorder.label')){
            case "foto":
                $img = "<img src='/".object_get($entry, $crud->get('reorder.label'))."' width='150' height=''>";
                echo '<div><span class="disclose"><span></span></span>'.$img.' <span style="margin-left: 100px;">'.$entry->title.' </span></div>';
                break;
            case "plugin_counter_id":
                $item = \App\Models\PluginCounter::where("id", object_get($entry, $crud->get('reorder.label')))->first();
                if($item){
                    echo '<div><span class="disclose"><span></span></span>'.$item->title.'</div>';
                }
                break;
            default:
