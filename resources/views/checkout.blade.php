...
@endsection
    ...
    @section('content')
        ...
        @if(!\Session::has('user_id'))
            @if(env('TEMA') == "Webshop")
                @include("$thema.plugins.pluginProducts.v3.checkout_nologin")
            @else
                @include("common.pluginProducts.checkout_nologin")
            @endif
        @else
            @if($shopSetting->checkout == 1)
                @if(env('TEMA') == "Webshop")
                    @include("$thema.plugins.pluginProducts.v3.checkout")
                @else
                    @include("common.pluginProducts.checkout")
                @endif
            @else
                @if(env('TEMA') == "Webshop")
                    @include("$thema.plugins.pluginProducts.v3.checkout_mini")
                @else
                    @include("common.pluginProducts.checkout_mini")
                @endif
            @endif
        @endif
