...
if($item){
    ...
    if($item->ultimi_inseriti){
        ...
        $list_ultimi = \App\Models\PluginProducts::orderBy("id", "desc")
            ->where("is_variant", 0)
            ->where("is_active", 1)->take($item->number_items)->get();
    }

    $list_vetrina = [];
    if($item->in_vetrina){
        $vet["vetrina"] = @$labels['shop-in-vetrina'] ;
        $list_vetrina = \App\Models\PluginProducts::orderBy("id", "desc")
            ->where("is_variant", 0)
            ->where("is_active", 1)->where("is_evidenza", 1)->take($item->number_items)->get();
    }
    ...
    if($item->piu_venduti){
        ...
        if(count($ids)){
            $list_venduti = \App\Models\PluginProducts::orderBy("id", "desc")
                ->where("is_variant", 0)
                ->where("is_active", 1)->whereIn("id", $ids)->take($item->number_items)->get();
        }
