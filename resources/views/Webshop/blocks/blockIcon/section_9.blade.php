        ...
        <div class="row g-0">
            @if($array)
                @foreach($array as $value)
                    ...
                    <div class="col-lg-{{ $item->col }} col-md-12 col-sm-12">
                        <div class="card shadow-none align-items-center text-center wow animate__fadeInUp" style="--ibox-bg: {{ $value->bgcolor }}; --ibox-bg-h: {{ $value->bgcolor_over }}; --ibox-txt-color: {{ $value->color_title }};" data-wow-duration=".3s">
                            @if($value->icon)
                                <div class="icon" style="--icon-color: {{ $item->color_icon }}; --icon-border-color: {{ $item->bgcolor_hover }}; --icon-bg-color: {{ $item->bgcolor_hover }};">
                                    @if($url != "#")
                            ...
                            <div>
                                @if($url != "#")
                                    <a target="{{ $type_href }}" href="{{ $url }}" style="align-items: center;">
                                        <h5 style="color:{!! $value->color_title !!};">{{ $title[\App::getLocale()] }}</h5>
                                        <div class="description">{!! $description[\App::getLocale()] !!} </div>
                                    </a>
                                @else
                                    <h5 style="color:{!! $value->color_title !!}; align-items: center;">{{ $title[\App::getLocale()] }}</h5>
                                    <div class="description" style="align-items: center;">{!! $description[\App::getLocale()] !!} </div>
                                @endif
