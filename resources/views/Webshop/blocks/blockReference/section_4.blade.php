                    ...
                    <div class="grid-item {{ \Str::slug($category[\App::getLocale()], '-')  }}">
                        <div class="card mb-4">
                            ...
                            @if(trim($title[\App::getLocale()]) != "" || trim($description[\App::getLocale()]) != "" )
                                <div class="card-footer" style="background-color:{{ $item->box_color }}!important;">

                                    @if(trim($title[\App::getLocale()]) != "")
                                        <h5 class="title" style="color:{{ $item->title_color }}!important;">{{ $title[\App::getLocale()] }}</h5>
                                        <div class="subtitle" style="color:{{ $item->subtitle_color }}!important;">{{ $description[\App::getLocale()] }}</div>
                                    @endif

                                    @if(trim($button[\App::getLocale()])!="")
                                        <a class="btn btn-primary" style="background-color: {{ $item->background_color_btn }}; border-color: {{ $item->border_color_btn }}; color: {{ $item-> text_color_btn }};" target="{{ $type_href }}" href="{{ $url }}">
                                            {{ $button[\App::getLocale()] }}
                                        </a>
                                    @endif
