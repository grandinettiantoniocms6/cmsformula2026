<div class="sidebar-widgets-wrap">
    <div class="sidebar-header">
        ...
        <button class="sidebar-close" data-toggle="hide" data-target="#sidebar-shop"><i class="fas fa-times"></i></button>
    </div>
    <div class="sidebar-widgets-body">
        <div class="sidebar-widget">
            <h5 class="sidebar-widget-title mb-20" data-bs-toggle="collapse" data-target="#widget-search" aria-expanded="true">{{ @$labels['cerca'] }}</h5>
            <div class="widget-search collapse show" id="widget-search">
                @include("$thema.plugins.pluginProducts.v3.inc.search")
            </div>
        </div>
        <div class="sidebar-widget">
            <h5 class="sidebar-widget-title mb-20" data-bs-toggle="collapse" data-target="#widget-categorie" aria-expanded="true">{{ @$labels['categorie'] }}</h5>
                            ...
                            @if($tot > 0)
                                <li>
                                    <div class="d-flex align-items-center">
                                        ...
                                        @if(count($category->figli) > 0)
                                            <i class="fas" data-bs-toggle="collapse" data-target="#cat_{{ $category->id }}" aria-expanded="false"></i>
                                        @endif
                                    </div>
                                    @if(count($category->figli) > 0)
        ...
        @if(count($tags) && $plugin->show_tags == 1)
            <div class="sidebar-widget">
                <h5 class="sidebar-widget-title mb-20" data-bs-toggle="collapse" data-target="#widget-tags" aria-expanded="true">{{ @$labels['tags'] }}</h5>
