...
@if($website->is_online == 1 || backpack_user() || is_numeric(strpos(env('APP_URL'), "stage")))
    ...
    @section('content')
        @if($thema == "Webshop")
            @include("$thema.myarea.company")
        @else
            @include("common.pluginProducts.myarea.company")
        @endif
            ...
            <script>
                function delete_address() {
                    Swal.fire({
                        ...
                        text: 'Vuoi eliminare questo indirizzo di Fatturazione?',
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            window.location.href = '/myarea/company_delete/{{ $company->id }}';
                        }
                    });
