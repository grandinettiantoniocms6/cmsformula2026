                ...
                @foreach($array as $value)
                        ...
                        ?>
                            ...
                            <div class="swiper-slide tj-slider-item">
                                <div class="slider-bg-image" data-bg-image="{{ $foto }}"></div>
                                <div class="container">
                                    <div class="slider-wrapper">
                                        <div class="slider-content">
                                            @if(trim($title[\App::getLocale()])!="")
                                                <h1 class="slider-title" style="color: {{ $value->title_background }};">{!! $title[\App::getLocale()] !!}</h1>
                                            @endif

                                            @if($abstract[\App::getLocale()])
                                                    <div class="slider-desc">{!! $abstract[\App::getLocale()] !!} </div>
                                            @endif

                                            @if(trim($button[\App::getLocale()])!="")
                                                <div class="slider-btn">
                                                    <a class="tj-primary-btn" style="background-color: {{ $website->btn_background }}; border-color: {{ $website->btn_colorborder }};" href="{{ $url }}" target="{{ $type_href }}">
