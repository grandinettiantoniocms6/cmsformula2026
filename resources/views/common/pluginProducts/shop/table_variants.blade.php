    ...
    <table class="table table-responsive" width="100%">
        ...
        @foreach($options as $option)
            <tr>
                ...
                @if(!$attribute_secondary)
                    <td><input type="number" class="form-control" min="1" max="20" value="0"></td>
                @else
                    @if($options_2)
                        @foreach($options_2 as $option_2)
                            <?php
                            ...
                            @if($item_p)
                                <td>
                                    ...
                                    <img src="{{ $cover }}" width="20" height="20" class="text-center">
                                        <input type="number" name="variants_ids[{{ $item_p->product_id }}]" min="0" max="{{ $variant_p->qty }}" value="<?php echo $value_qty;?>">
                                </td>
                            @else
                                <td></td>
                            @endif
        ...
        <button class="btn btn-block">{{ @$labels['shop-add-to-cart'] }}</button>
    </form>

    @if($cont > 0)
        <br><a href="{{ route('cart') }}" class="btn btn-lg btn-outline-primary btn-block px-1 px-md-2"><span class="text">{{ @$labels['shop-varie-hai-nel-carrello'] }} {{ $cont }} {{ @$labels['shop-varie-prodotti-vai-carello'] }}</span></a>
    @endif
@endif
