...
?>
    ...
    @if($cont > 0)
        <a href="{{ route('cart') }}" class="btn btn-lg btn-outline-primary btn-block my-3"><span class="text">{{ @$labels['shop-varie-hai'] }} {{ @$labels['shop-varie-prodotti-di'] }} <strong>{{ $itemProduct->name_variant }}</strong> {{ @$labels['shop-varie-nel-carrello'] }}</span></a>
    @endif
