...
@if($array)
    @foreach($array as $value)
            ...
            if($photo_mobile2){
                if($agent->isMobile() || $agent->isTablet()){
                    if($value->foto_mobile_2){
                        $photo_mobile1 = $value->foto_mobile_2;
                    }else{
                        $photo_mobile1 = $value->sfondo;
                    }
                }else{
        ...
        <section class="block-hero style-{{ $item->style }} image-wrapper bg-overlay bg-overlay-black-{{ $item->alpha }}" id="block-hero-{{ $value->block_id }}" style="background-color: {{ $value->bgcolor }}!important;">
            <div class="container">
                <div class="row d-flex align-items-center wow animate__fadeInUp" data-wow-duration=".3s">
                    @if($foto)
                        <div class="col-md-6 col-xl-7 mb-3 mb-md-0">
                            <div class="hero-img-block">
                                <picture>
                                    @if($agent->isMobile() || $agent->isTablet())
                                        ...
                                        @endif
                                    @endif
                                    <img class="img-fluid jump" src="{{ $foto }}" alt="{{ $title[\App::getLocale()] }}" loading="lazy">
                                </picture>
                            </div>
                        </div>
                    @endif
