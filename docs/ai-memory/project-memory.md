...
- Anche l'import prodotti normale (`/admin/plugin/pluginProducts/import_export`, form "Import") deve passare dalla coda `database_imports` e dalla run persistita in `plugin_product_import_runs`: usare `plugin_product_import_id = null` per distinguerlo dallo SPECIAL, salvare opzioni operative in `import_options`, e lasciare al job `ProcessPluginProductImport` gli step automatici `set:products_search` e `set:products_categories_search`.
- La pagina import/export prodotti puo' accodare anche una run senza file per eseguire solo `set:products_search` e `set:products_categories_search`: usare `import_options.only_search_commands = true`, nessuno step `import`, e lasciare il monitor run come fonte dello stato.
...
- Nella login Backpack custom (`resources/views/vendor/backpack/theme-coreuiv2/auth/login.blade.php`), i toggle password basati su link ad ancora (`href="#password"`) devono sempre chiamare `event.preventDefault()`: altrimenti il click sull'icona occhio modifica hash/scroll e sposta il layout del form.
...
- Nel listing prodotti frontend, evitare lookup per singola opzione/brand durante la costruzione filtri sidebar (`ShopAttributesOptions::find()`, `PluginProductsBrands::find()` in loop) e cacheare solo a runtime per richiesta settings/plugin/conteggio promozioni usati da `PluginProducts::get_promo_price()`: le ripetizioni generano centinaia di query N+1.
- Per velocizzare il primo hit delle categorie prodotti, usare la tabella ponte indicizzata `plugins_products_search_categories` generata da `set:products_search` invece dei filtri testuali `categories LIKE '%,id,%'` / `langs LIKE '%,it,%'`; il controller mantiene fallback al vecchio campo se la migration non e' presente.
...
- Nei form frontend Crafto generati da `blockContact`/`blockPluginForm`, i controller inviano redirect con flash message: non usare la classe `.submit` del tema se il controller non restituisce JSON, altrimenti l'AJAX Crafto invia il messaggio ma non mostra feedback all'utente.
...
- Nei partial frontend Crafto inclusi dopo script con `defer`, non usare handler immediati basati su `$`/jQuery: registrare listener nativi (`window.addEventListener`) e inizializzare Bootstrap al `load`, con fallback jQuery solo se gia disponibile. Per popup con range date admin, considerare `popup_end` inclusivo fino a fine giornata.
