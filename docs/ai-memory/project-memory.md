...
- Su Crafto `inc_multilang`, `icon.min.css` e GLightbox non sono risorse critiche per il primo render mobile: caricare `icon.min.css` via preload/onload e GLightbox solo quando la pagina ha blocchi lightbox, con guard `typeof GLightbox` in `main.js`.
