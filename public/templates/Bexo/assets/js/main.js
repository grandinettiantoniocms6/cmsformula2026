...
(function ($) {
	...
	function normalizeMenuPath(rawUrl) {
		if (!rawUrl || rawUrl === "#" || rawUrl.indexOf("javascript:") === 0) {
			return null;
		}
	...
	function getVisitedMenuColor() {
		var cssColor = window.getComputedStyle(document.documentElement)
			.getPropertyValue("--header-link-visited-color")
			.trim();
		return cssColor || null;
	}
	function forceVisitedColor($link, color) {
		if (!color) {
			return;
		}
		var el = $link.get(0);
	...
	function markVisitedMenuLinks() {
		var currentPath = normalizeMenuPath(window.location.href);
		var visitedColor = getVisitedMenuColor();
		$("#mobile-menu a[href], .mean-container .mean-nav a[href]").each(function () {
			var normalizedPath = normalizeMenuPath($(this).attr("href"));
			if (!normalizedPath) {
				return;
			}
			var isCurrent = currentPath && (
				currentPath === normalizedPath ||
				(normalizedPath !== "/" && currentPath.slice(-normalizedPath.length) === normalizedPath)
			);
			if (isCurrent) {
				$(this).addClass("is-current-menu-link");
				forceVisitedColor($(this), visitedColor);
			}
		});
	}
	markVisitedMenuLinks();
	$(window).on("load", markVisitedMenuLinks);

	// Evita chiusura immediata: su alcuni device il tap di apertura
	// puo propagarsi all'overlay nello stesso frame.
	var hamburgerJustOpened = false;
	function closeMenus() {
	...
	$(".mobile_menu_bar").on("click", function (e) {
		...
		hamburgerJustOpened = true;
		setTimeout(function () {
			hamburgerJustOpened = false;
		}, 250);
	});

	// Offcanvas js
	$(".menu_offcanvas").on("click", function (e) {
		e.preventDefault();
		e.stopPropagation();
		var shouldOpen = !$(".tj-offcanvas-area").hasClass("opened");
		$(".tj-offcanvas-area").toggleClass("opened", shouldOpen);
		$(".body-overlay").toggleClass("opened", shouldOpen);
		$("body").toggleClass("overflow-hidden", shouldOpen);
	});
	$(".hamburger_close_btn").on("click", function (e) {
		e.preventDefault();
		closeMenus();
	});
	$(".body-overlay").on("click", function () {
		if (hamburgerJustOpened) {
			return;
		}
		closeMenus();
	});
